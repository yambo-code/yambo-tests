...
if ($np==1 or $MPI_CPU_conf[1] eq "serial") {
 $command_line = "$nice $yambo_exec $INPUT_file $flags $in_dir_cmd_line $force_serial $log";
}else{
 $command_line = "$nice $mpiexec -np $np $yambo_exec $INPUT_file $flags $in_dir_cmd_line $force_serial $CPU_cmd $log";
}
#
# *** RUN ***
