 ...
 print <<EndOfUsage
   ...
   Syntax: driver.pl [OPTIONS]
         ...
         [OPTIONS] are:
                   ...
                   -p      [PJ]         Test only this project (e.g. sc,none,any)
                   -n                   Perform dry run
                   -c                   Clean
...
if($listtests or $clean ) {
  ...
  foreach $dir (<*>,<*/*>,<*/*/*>,<*/*/*/*>,<*/*/*/*/*>) {      # Glob all files
    ...
    if (-d $dir."/SAVE" && -d $dir."/INPUTS" ) {
      ...
      #
      # Loop through all files inside INPUTS (conf/actions/flags/00_*)
      # Store yambo input file in $SUB_DIR, and clean, otherwise skip (!)
      #
  ...
  #
  if (  $clean) {
    $system_command = "svn st | grep -v 'ns.' | grep -v 'ndb.elph_gkkp' | grep -v '.gz' | grep '?' | awk '{print $2}' | xargs rm -fr";
    print $system_command."\n" if ($verb);
    system ($system_command);
  };
}
...
#
$is_GPL="no";
if ($branch=~m/gpl/ix or $branch=~m/trunk_stable/ix) {$is_GPL="yes"};
#
...
#
print tlog "***** Summary of test options *****\n\n";
#
# date
#
...
#
foreach $testline (@input_tests_list) {
   #
   ...
   chdir($testdir) or die("$!, stopped");
   #
   # clean up old shit
   #
   unless($savefiles) {&clean()};
   foreach $testname (@inputs) {
     if($testname =~ /\*/) {die "Wildcard operator found in tests: dangerous!"};
   ...
   foreach $testname (@inputs) {
     ...
     if( -e "INPUTS/$testname.conf") {
       ...
       #
       $description = shift(@conf_file);
       $description = substr($description,6);
       chomp($description);
       #
       # ... test tolerance, overrides user defined value
       #
       &set_tolerance;
       #
     }
     ...
     &do_user_actions;
     #
     # Check if etra flags are needed
     #
     &add_user_flags;
     #
     ...
     LOOP: for (my $ir=1; $ir<=$Nr ; $ir++){
       ...
       foreach $dir (@inputs) {
         #print $dir;
         #if (-e "$dir-$ir") {
         #  $command_line = "rm -fr $dir; ln -s $dir-$ir $dir";
         #  $return_value = system("$command_line");
         #  #print "$command_line\n";
         #}
       }
       #
       print tlog ">> Configuration is $string($ir)\n";
       #
       ...
       #
       $command_line = "mkdir $dir_name";
       $return_value = system("$command_line");
       $command_line = "cp -f yambo.in $dir_name";
       $return_value = system("$command_line");
       foreach $file (<o-*$testname*>) {
         $command_line = "mv -f $file $dir_name";
         $return_value = system("$command_line");
       }
       foreach $file (<r-*$testname*>,<ref_*>,<run_*>) {
         $command_line = "mv -f $file $dir_name";
         $return_value = system("$command_line");
       }
       foreach $file (<l-*$testname*>) {
         $command_line = "mv -f $file $dir_name";
         $return_value = system("$command_line");
       }
       foreach $file (<LOG/l*$testname*>) {
         $command_line = "mv -f $file $dir_name";
         $return_value = system("$command_line");
       }
       #
...
sub get_actions_from_input_file
{
 ...
 foreach $file (<REFERENCE/o-$testname.*>){
   $file =~ s{.*/}{};
   if (!-e "$file") { 
   ...
   if ( $file=~ /.hf/ and $is_GPL=~"yes" ) { $comp_it = "no"};
...
sub run_it
{
 ...
 $return_value = system("$command_line") if ($verb);
 if ($Ncpu==1) {
...
sub comp_action
{
 ...
 $system_command = "$scripts_dir/find_the_diff -r $ref_filename -o $run_filename";
 if($verb) {print "Running floating point test: $system_command \n\n";}
 $fldiff_output = `$system_command`;
 #print "$fldiff_output";
 #
 if( $fldiff_output =~ /OK/) {
  ...
  $passes++;
   ...
   #@fldiff_out = split("\n",$fldiff_output);
   #$last = $fldiff_out[$#fldiff_out];
   #@last_splitted= split(" ",$last);
   ##print $fldiff_output;
   ##@ABS_=substr($last_splitted[9],0,5);
   #$COL="$last_splitted[14]";
   ##print "$last $COL $file\n";
   #@ABS_=$last_splitted[9];
   #@REL_=substr($last_splitted[12],0,5);
   #$REPORT = "N=$last_splitted[4] ABS=$ABS_[0] REL=$REL_[0]%";
   #if ($last_splitted[3] =~ "fatal" or $last_splitted[3] =~ "Unable") {
   # $REPORT = "FATAL error"; 
   # $REL_[0]=2;
   # $ABS_[0]=1;
   #} 
   ##
   ## For dynamics files skip Temperatures fields
   ##
   #if ($ABS_[0] < $UP_lim_on_diff or ($file=~m/dynamics/ix and $COL>5) or ($file=~m/0K*mean_EElifetimes/ix and $COL>3) or ($file=~m/plasma.mean_EElifetimes/ix and $COL>3) or ($file=~m/300K*mean_EElifetimes/ix and $COL>5) or ($file=~m/mean_EPlifetimes/ix and $COL>5) ) {
   # print tlog "\t [ $color_start{green}  OK  $color_end{green} ] \n"; 
   # $test_succeeded = 1;
   # $passes++;
   #} else {
   # print tlog "\t [ $color_start{red} FAIL $REPORT $color_end{red} ] \n"; 
   # $test_succeeded = 0;
   # if ($fail=~"no") {$fail="yes"};
   # $failures++;
   #}
   #print "$testdir-$branch/$dir_name/$run_filename:\t $fldiff_output"; 
   print elog "$testdir-$branch/$dir_name/$run_filename:\t $fldiff_output\n" if ($elapsed < $run_duration); 
   print tlog "\t [ $color_start{red} FAIL $color_end{red} ] $fldiff_output"; 
   $test_succeeded = 0;
   if ($fail=~"no") {$fail="yes"};
   $failures++;
 }
