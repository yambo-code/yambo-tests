...
if ($upload) {
 ...
 if ($filesSize>0) 
 {
  my $dir = (split(/\//, $suite_dir))[-1];
  &command("find . -name 'ns.*' -o -name 'ndb.elph*' | xargs tar cvf $dir.tar");
  &command("gzip $dir.tar");
  &command("$ncftpput -u 1945528\@aruba.it -p 5fv94ktp ftp.yambo-code.org www.yambo-code.org/$where $dir.tar.gz");
 }
 &command("$ncftpput -u 1945528\@aruba.it -p 5fv94ktp ftp.yambo-code.org www.yambo-code.org/$where $dir.tar.gz");
 die "\n";
}else{
